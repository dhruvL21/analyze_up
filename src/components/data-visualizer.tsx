...
export function DataVisualizer() {
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      ...
      // Helper to check page bounds and handle pagination
      const checkPageBreak = (neededHeight: number) => {
        if (cursorY + neededHeight > pageHeight - 20) {
          // Draw footer on current page before moving to next page
          pdf.setFontSize(8);
          pdf.setTextColor(150, 150, 150);
          pdf.text(
            'This report was automatically generated by AnalyzeUp AI. Strategic decisions should be verified with complete financial audits.',
            pageWidth / 2,
            pageHeight - 10,
            { align: 'center' }
          );
          ...
          pdf.addPage();
          cursorY = margin + 10; // Reset cursor Y on subsequent pages
        }
      };
      ...
      const stats = [
        `Total ${metric}: Rs. ${total.toLocaleString('en-IN')}`,
        `Average ${metric}: Rs. ${avg.toLocaleString('en-IN')}`,
        `Peak ${metric}: Rs. ${max.toLocaleString('en-IN')}`,
        `Lowest ${metric}: Rs. ${min.toLocaleString('en-IN')}`
      ];
      ...
      // Key Observations
      const allObsLines = insights.keyObservations.map(obs => {
        const obsLines = pdf.splitTextToSize(obs, pageWidth - (margin * 2) - 10);
        return { text: obsLines, height: (obsLines.length * 5) + 2 };
      });
      
      checkPageBreak(18); // Header + first line space
      pdf.setFont('helvetica', 'bold');
      pdf.text('Key Observations:', margin, cursorY);
      cursorY += 6;

      pdf.setFont('helvetica', 'normal');
      allObsLines.forEach(obs => {
        checkPageBreak(obs.height);
        pdf.text(`• `, margin + 2, cursorY);
        pdf.text(obs.text, margin + 7, cursorY);
        cursorY += obs.height;
      });
      ...
      // Recommendations
      const allRecLines = insights.recommendations.map(rec => {
        const recLines = pdf.splitTextToSize(rec, pageWidth - (margin * 2) - 10);
        return { text: recLines, height: (recLines.length * 5) + 2 };
      });
      ...
      pdf.setFont('helvetica', 'normal');
      allRecLines.forEach(rec => {
        checkPageBreak(rec.height);
        pdf.text(`→ `, margin + 2, cursorY);
        pdf.text(rec.text, margin + 7, cursorY);
        cursorY += rec.height;
      });
