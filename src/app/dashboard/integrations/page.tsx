      ...
      if (fileType === 'SALES_REPORT' || validRows.some(r => r.parsed.orderNo)) {
        ...
        validRows.forEach((r, idx) => {
          ...
          transactionsToImport.push({
            ...
            status: (r.parsed.status && /pending|unfulfilled|placed/i.test(r.parsed.status)) ? 'Pending' : 'Completed',
            fulfillmentStatus: (r.parsed.status && /pending|unfulfilled|placed/i.test(r.parsed.status)) ? 'UNFULFILLED' : 'FULFILLED',
            financialStatus: (r.parsed.paymentStatus && /unpaid|pending/i.test(r.parsed.paymentStatus)) ? 'PENDING' : 'PAID',
            deliveryStatus: (r.parsed.status && /pending|unfulfilled|placed/i.test(r.parsed.status)) ? 'PENDING' : 'DELIVERED',
            isRevenueRecognized: !(r.parsed.status && /pending|unfulfilled|placed/i.test(r.parsed.status)),
            paymentReceived: !(r.parsed.paymentStatus && /unpaid|pending/i.test(r.parsed.paymentStatus)),
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs md:text-sm text-muted-foreground mt-1">
            Connect your Shopify storefront, Google Drive automated spreadsheet sync, or import offline CSV/Excel catalogs to power AI inventory intelligence.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-semibold text-foreground flex items-center gap-1.5">
          ...
          Core Ingestion & Storefront Channels
        </h3>
